...
class Product(db.Model):
    id: int = db.Column(db.Integer, primary_key=True)
    seller_id: int = db.Column(db.Integer, db.ForeignKey('seller.id'),
                                         nullable=False)
    ...
    picture: str = db.Column(db.String(255), default=None)
    open_datetime: datetime = db.Column(db.DateTime, nullable=False)
    selled_datetime: datetime = db.Column(db.DateTime, default=None)
    is_selled: bool = db.Column(db.Boolean, default=False)

    def __init__(self, seller_id, name, description, type, cost, picture=None):
